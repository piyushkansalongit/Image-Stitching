...
class transformExtactor:
    ...
    def RANSAC(self, i, j, key_1, key_2, set_1, set_2):
        ...
        if len(good)>MIN_MATCH_COUNT:
            ...
            affine, mask = cv2.estimateAffine2D(src_pts, dst_pts, cv2.RANSAC,ransacReprojThreshold=5.0)
        ...
        np.save(os.path.join(self.affine_path, images[i].replace(".jpg",'')+"-"+images[j].replace(".jpg",'')), affine)
